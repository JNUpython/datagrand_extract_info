...
from prepare_data import get_voc_dict, get_tag_dict
...
#     """
#     logger.info("对特征和输出的[%s]建立vocabulary, data: %s" % (", ".join(columns), path_data))
...
#         # 因为长度不够， 序列存在填充， 训练word2vec时候已经考虑了未登录词汇，所以这里不需要考虑
#         logger.info(feature_item_dict_list[i])
#         logger.info(len(feature_item_dict_list[i]))
#         size = create_dictionary(
...
def main():
    ...
    voc_sizes = [get_voc_dict(["../data/train.txt", "../data/test.txt"], 2), get_tag_dict("../data/lstm_crf/train.txt", 1)]
    lengths = [128]
    ...
    for i, feature_name in enumerate(feature_names):
        ...
        config['model_params']['embed_params'][feature_name]['path'] = "../data/lstm_crf/%s_embed.mat.pkl" % feature_name
        ...
        embedding_matrix = np.zeros((len(voc.keys()) + 2, vec_dim), dtype='float32')  # 第一行为填充
    ...
    for i, feature_name in enumerate(feature_names):
        config['model_params']['embed_params'][feature_name]['shape'] = [voc_sizes[i], feature_dim_dict[
            feature_name]]  # 保存特征集合大小 和 embedding size
    # 修改char表的embedding
    if use_char_feature:
        # 默认16维，根据任务调整
